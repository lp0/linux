...
static void bcm6345_l1_irq_handle(struct irq_desc *desc)
{
	...
	for (i = 0; i < intc->n_words; i++) {
		int base = i * IRQS_PER_WORD;
		unsigned long pending;
		irq_hw_number_t hwirq;
		unsigned int irq;

		/*
		 * This does not need to hold intc->lock because it
		 * doesn't access intc->cpus[]->enable_cache[].
		 */
		pending = __raw_readl(cpu->map_base + reg_status(intc, i));
		pending &= __raw_readl(cpu->map_base + reg_enable(intc, i));
			      ...
			      struct device_node *parent)
{
	...
	for_each_possible_cpu(cpu_idx) {
		ret = bcm6345_l1_init_one(dn, cpu_idx, intc);
		if (ret)
			pr_err("failed to init intc L1 for cpu %d: %d\n",
				cpu_idx, ret);
		else
			cpumask_set_cpu(cpu_idx, &intc->cpumask);
	}
	...
	for_each_cpu(cpu_idx, &intc->cpumask) {
